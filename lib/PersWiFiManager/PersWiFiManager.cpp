...
void PersWiFiManager::handleWiFi() {
  ...
  if ((WiFi.status() == WL_CONNECT_FAILED) || 
      _freshConnectionAttempt || 
      ((WiFi.status() != WL_CONNECTED) && ((millis() - _connectStartTime) > (1000 * _timeout))))
  {
    startApMode();
...
void PersWiFiManager::setTimeout(uint32_t timeout)
{
  _timeout = timeout ;
} // setTimeout
...
void PersWiFiManager::onAttempt(WiFiChangeHandlerFunction fn) {
  _attemptHandler = fn;
}
